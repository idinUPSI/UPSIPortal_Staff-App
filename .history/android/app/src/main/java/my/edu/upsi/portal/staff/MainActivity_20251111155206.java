                ...
                if (getBridge() != null && getBridge().getWebView() != null) {
                    ...
                    webView.setWebViewClient(new WebViewClient() {
                        ...
                        public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
                            // Only handle main frame network errors
                            if (request.isForMainFrame()) {
                                int errorCode = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ? error.getErrorCode() : -1;
                                
                                // Check if it's a network connectivity error
                                boolean isNetworkError = (errorCode == ERROR_HOST_LOOKUP ||
                                                         errorCode == ERROR_CONNECT ||
                                                         errorCode == ERROR_IO ||
                                                         errorCode == ERROR_TIMEOUT ||
                                                         errorCode == -2); // ERR_INTERNET_DISCONNECTED
                                ...
                                if (isNetworkError) {
                                    ...
                                    return; // Don't call super - we handle this ourselves
                                }
                            }
                            // For all other errors, let WebView handle normally
